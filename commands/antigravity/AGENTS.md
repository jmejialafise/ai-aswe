...
* **Long-Term Brain (`.ai/memory/`):** Your DNA and static knowledge.
    ...
    * Completed feature specs are archived in `archive/` (e.g. after `/spec-log`).
...
## 4. COMMAND AWARENESS
The human developer will interact with you using custom commands defined in `.agent/workflows/` (e.g. `/plan`, `/plan.summarize`, `/spec-define`, `/fast`). In Antigravity, these appear when typing `/` in chat.
